...
## 3. O que eu aprendi
### Giselly Jahel Dias
Nesta semana aprendi como um microcontrolador lê sinais externos através de entradas digitais, e como usar botões em um sistema.
Aprendi que uma entrada digital é um pino do microcontrolador que lê o estado elétrico externo e o interpreta como apenas dois valores: HIGH (1) ou LOW (0). Aprendi que quando um pino não está conectado a nada, ele fica flutuando, ou seja, sem referência de tensão definida, ele capta ruído elétrico do ambiente e alterna aleatoriamente entre HIGH e LOW, gerando leituras imprevisíveis e comportamento incorreto no sistema. Para evitar isso, aprendi que é necessário utilizar um resistor que define um estado padrão estável para o pino. O pull-down conecta o pino ao GND (padrão LOW); quando o botão é pressionado, o pino é puxado para HIGH. O pull-up faz o oposto: conecta o pino ao VCC (padrão HIGH); quando o botão é pressionado, o pino vai para LOW. Também aprendi que botões físicos sofrem debounce: ao serem pressionados, os contatos mecânicos vibram por alguns milissegundos, gerando múltiplos pulsos onde deveria haver apenas um. Dessa forma, o sistema registraria vários acionamentos por clique, mas o debounce por software resolve isso aguardando o sinal estabilizar por um período fixo (tipicamente 10–50ms) antes de registrar a mudança de estado como válida.
...
## 7. Código
O arquivo principal do firmware é ligabotao.ino, localizado em firmware/arduino/semana-02.1.ino
```cpp
